...
namespace Formation
{
    ...
    {
        [SerializeField] bool CenterOnUnit;
        IFormationData _data;
        public Vector3 GetModelPosition(int index)
        {
            Vector3 offset = Quaternion.AngleAxis(transform.eulerAngles.z, Vector3.forward) * GetModelOffsetPosition(index);
        ...
        public Vector3 GetModelOffsetPosition(int index)
        {
            ...
            float y;
            if (CenterOnUnit)
                y = _data.ModelCount / _data.Width / 2 - (float)(index / _data.Width);
            else
                y = -(index / _data.Width);
